...
class CRM_Mafsepa_Logic_Group {
  ...
  static function close($txgroup_id) {
    ...
    if ($txgroup['type']=='OCR') {
      // Update the first contribution id column only when the column is empty
      ...
      $sql = "UPDATE civicrm_sdd_mandate
              ...
              WHERE civicrm_sdd_mandate.entity_table = 'civicrm_contribution_recur'
                AND civicrm_sdd_mandate.first_contribution_id IS NULL
    ...
    $ocrFile = civicrm_api('SepaAlternativeBatching', 'createocr', array('txgroup_id'=>$txgroup_id, 'version'=>3));
    if (isset($ocrFile['is_error']) && $ocrFile['is_error']) {
      $lock->release();
      return "Cannot create OCR file for group ".$txgroup_id;
    }
   ...
   */
  public static function cleanup($type) {
    $group_status_id_open = (int) CRM_Core_OptionGroup::getValue('batch_status', 'Open', 'name');
